...
//        handler : 화면단에서 적어주는, 응답을 어떻게 처리할 지 대처 방안이 담긴 인터페이스 변수.

//        로그인기능
        fun postRequestLogin(email: String, pw: String, handler : JsonResponseHandler? ) {

//            1.어디로 요청하러 (인터넷 주소 연결 - URL) 갈 것인가??
            val urlString = "${ BASE_URL}/user"

//            2. 파라미터를 어떻게 들고 갈 것 인가?? - POST : formData 활용
...
//                    실패 : 서버에 연결 자체를 실패. 아무 응답도 없을 때.
//                    ex. 인터넷 끊김, 서버 접속 불가 등등 물리적 연결 실패
//                    비번 틀려서 로그인 실패 : 연결은 되었고, 응답도 잘 돌아왔는데 > 그 내용만 실패(응답한게맞음)
                }

                override fun onResponse(call: Call, response: Response) {

//                    어떤 내용이든, 응답 자체가 잘 들어온 경우.(로그인 성공, 실패 모두 응답은 돌아온게 맞음)
...
//            1. 어디로 가야하는가? GET-query 파라미터 > 어디로? + 어떤데이터? 한번에 조합된 형태
//              >> 만들때도 같이 만들어야함함
...
//            2. 어디로 + 어떤데이터? 완료된 상태 > Request 만들자
            val request = Request.Builder()
                ...
                .get() // get방식은 파라미터 변수를 따로 안받음. url에 다 있기때문에
                .build()
...
//        대부분의 서버 관련 함수는 context : Context를 재료로 맨 앞에 받아두면 편하다.
...
//            urlBuilder.addEncodedQueryParameter() // 이 기능은 쿼리파라미터 첨부가 없음.
...
//            Path 첨부 : /topic/3 양식 > 이름표작성 X.
            urlBuilder.addPathSegment(topicId.toString())

//            Query 첨부 : 주소 ? order_type=NEW 양식 > 이름표작성 O
...
//            urlBuilder.addEncodedQueryParameter() // 이 기능은 쿼리파라미터 첨부가 없음.
        ...
        fun postRequestWriteReReply(context: Context, content:String, parentReplyId:Int,handler: JsonResponseHandler? ){
            val urlString = "${BASE_URL}/topic_reply"
