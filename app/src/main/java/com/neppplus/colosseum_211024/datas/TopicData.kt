...
//        다른 화면에서는 jsonObj만 파싱해서 > 변환기능으로 활용만
...
//            jsonObj 내부에 sides JSONArray가 있다 > 파싱 : SideData 목록에 추가
            ...
            for( i in 0 until sidesArr.length() ) {
                ...
                val sideData = SideData.gerSideDataFromJSON(sideObj)
...
//            null인지 아닌지 확인해서 파싱 진행. => null이 아닐 때 파싱
            if(!jsonObj.isNull("my_side")){

                val mySideObj = jsonObj.getJSONObject("my_side")
                topicData.mySide = SideData.gerSideDataFromJSON(mySideObj)
            }
